...
import { createRollbackAction } from "@/app/actions/events";
import { ROLLBACK_COLOR } from "@/lib/deployStatusMeta";
...
export function RollbackModal({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await createRollbackAction({
        eventId, path, comment: comment || undefined,
      });
      if (res.ok) {
        onDone();
        router.refresh();
    ...
    <div className={styles.overlay} onClick={pending ? undefined : onClose}>
      <div ref={modalRef} className={styles.modal} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.actions}>
          ...
          <button className={styles.confirm} onClick={submit} disabled={pending}>
