...
export function LogsClient({ mailpitUrl }: { mailpitUrl: string | null }) {
  const { t } = useI18n();
  ...
  // Only when RC_MAILPIT_URL is set, which no deployed instance sets: the
  // catch-all mailbox is a development tool, and a link to one that is not
  // there would be worse than no link.
  const mailbox = mailpitUrl ? (
    <a
      href={mailpitUrl}
      target="_blank"
      rel="noreferrer"
      style={{ fontSize: "0.85em", color: "var(--accent)", marginLeft: "auto" }}
    >
      {t("logs.mailbox")} ↗
    </a>
  ) : null;
