          ...
          "application/json": {
            schema: {
              ...
              properties: {
                ...
                environment: { type: "string", nullable: true, description: "An environment slug, or `group:<slug>` standing for that group's member environments. A group that is deleted or emptied empties the feed rather than widening it." },
                mergeLots: { type: "boolean", description: "Serve one event per lot (spanning its members) instead of one per member service. Lots of a single member are unaffected. " + ignoredIfWrongType },
              },
            },
          },
        },
