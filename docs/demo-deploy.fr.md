...
*Français — [English version](demo-deploy.md)*
...
Rien n'est compilé sur le serveur : les deux images sont tirées de GHCR. `ci.yml` les
publie sur un push de `rc` ou `main` ; les versionnées sortent de
`release-please.yml` au merge de la PR de release. Voir [ci-cd.fr.md](ci-cd.fr.md).
...
merge de la PR de release ouverte par `release-please` sur `main` — pas d'un
`git tag` manuel.
...
`RC_IMAGE_TAG=rc`, la démo suit la branche RC ; avec `RC_IMAGE_TAG=0.2.0`, elle
